<script lang="ts">
  ...
  import Avatar from './avatar/Avatar.svelte';
    ...
    const currentEdges = writable(graph.filter(edge => edge.from === startNode));
    ...
    function moveToNode(target:any) {
        ...
        if (targetEdges.length > 0) {
            breadcrumbs.update(bc => [...bc, { node: target, label: shortenAddress(target) }]);
...
{#if $breadcrumbs.length > 1}
    <div class="breadcrumbs">
        {#each $breadcrumbs as crumb, index}
            <button on:click={() => navigateBackTo(index)} class="btn btn-link text-blue-500">
                {crumb.label}
            </button>
    ...
    {#each $currentEdges as edge}
        <div class="flex items-center justify-between p-2 bg-base-100 hover:bg-base-200 rounded-lg cursor-pointer"
            ...
            <div class="flex items-center gap-2">
                <div class="font-medium">{parseFloat(formatEth(edge.value)).toFixed(2)} Circles</div>
